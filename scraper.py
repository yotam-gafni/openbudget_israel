	    ...
	    print "Another hcode and title !"
            ...
            for item_code in item_codes:
                ...
                for x in rows:
                  ...
                  x['num2'] = 0
                  x['num3'] = 0
                  scraperwiki.sqlite.save(unique_keys=['item_code'], data=x)
#
                ...
                for row in frame("TR"):
                    ...
                    for x in row("TD.x3_0, TD.x3_1, TD.x2_0, TD.x2_1"):
                        ...
                        x=x.text()
                        try:
                            _row['num' + eval(str(i))] = int(x.replace(",", ""))
                            #_row.append(int(x.replace(",","")))
                        except:
                            _row['num' + eval(str(i))] = x.encode('utf8')
                            #_row.append(x.encode('utf8'))
                        i = i + 1
                    if len(_row.keys()) > 4:
                        #out.writerow(_row)
                        scraperwiki.sqlite.save(unique_keys=['item_code'], data=_row)
                        print _row

if __name__ == "__main__":
  # range(2012, 2015)
  for year in range(2012, 2013):
	 runner = scrape_supports().process(year)
